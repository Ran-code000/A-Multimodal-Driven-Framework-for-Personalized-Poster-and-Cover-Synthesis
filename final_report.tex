...
% ---- Color Palette ----
\usepackage{xcolor}
\definecolor{primary}{HTML}{1a3a5c}       % Dark navy — headings, cover bars, table headers
\definecolor{secondary}{HTML}{2c6faa}     % Medium blue — subheadings, borders, links
\definecolor{accent}{HTML}{76b900}        % Green accent — highlights, tips, cover accent
\definecolor{lightbg}{HTML}{f0f4f8}       % Light blue-gray — box backgrounds
\definecolor{tabhead}{HTML}{1a3a5c}       % Table header bg (= primary)
\definecolor{tabrowb}{HTML}{e8eff5}       % Alternating table row (light blue)
\definecolor{tabrowa}{HTML}{ffffff}       % White table row
\definecolor{codebg}{rgb}{0.97,0.97,0.98} % Code block background
\definecolor{warncol}{HTML}{e07b39}       % Warning orange
\definecolor{redcol}{HTML}{d94040}        % Error red
\definecolor{greencol}{HTML}{3a9d3a}      % Success green

% ---- Page Layout ----
\usepackage[top=2.5cm,bottom=2.5cm,left=2.6cm,right=2.6cm,headheight=26pt]{geometry}
\usepackage{setspace}
\setstretch{1.1}
\usepackage{indentfirst}
\setlength{\parindent}{2em}

% ---- Float spacing (prevent crowding) ----
...
\usepackage{fancyhdr}
\usepackage{tikz}
\usetikzlibrary{positioning,arrows.meta,shapes,calc,backgrounds,shapes.geometric,fit}
\usepackage{float}
\usepackage{caption}
\usepackage{subcaption}
\usepackage[colorlinks=true,linkcolor=black,citecolor=black,urlcolor=secondary]{hyperref}
\usepackage{listings}
\usepackage{titlesec}
...
\titleformat{\section}
    ...
    {\color{primary}\thesection}{0.8em}{}
\titleformat{\subsection}
    {\color{secondary}\normalfont\Large\bfseries}
    {\color{secondary}\thesubsection}{0.8em}{}
\titleformat{\subsubsection}
    {\color{primary}\normalfont\large\bfseries}
    {\color{primary}\thesubsubsection}{0.6em}{}
...
\lstdefinestyle{pycode}{
    ...
    frame=single,framerule=0.5pt,rulecolor=\color{codegray!50},
    language=Python,
    columns=flexible,
    tabsize=2,
    xleftmargin=4pt,
...
\newtcolorbox{keybox}[1][]{
    colback=lightbg,colframe=secondary,coltitle=white,
    fonttitle=\bfseries,title={\textbf{>}\enspace#1},
    sharp corners,boxrule=0.8pt,
    left=3mm,right=3mm,top=2mm,bottom=2mm
}
\newtcolorbox{warnbox}{
    colback=redcol!5,colframe=redcol!60,
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\color{secondary}\small 多模态大模型原理与应用·期末结题报告}
\fancyhead[R]{\color{secondary}\small 图文驱动个性化封面/海报生成}
\fancyfoot[C]{\color{secondary}\small\thepage}
...
\begin{tikzpicture}[remember picture,overlay]
    ...
    \fill[accent] (current page.north west) rectangle
        ([yshift=-0.65cm]current page.north east);
\end{tikzpicture}

\vspace*{2.5cm}

\begin{center}
    {\color{primary}\fontsize{28}{34}\bfseries 图文驱动的个性化封面\\[8pt]与海报生成系统}\\[14pt]
    {\color{secondary}\fontsize{16}{20}\bfseries 多模态大模型原理与应用·课程大作业期末结题报告}
\end{center}

\vspace{1.2cm}
\begin{center}\color{secondary}\rule{0.5\textwidth}{0.8pt}\end{center}
\vspace{1.2cm}

% Info card
\begin{center}
\begin{tikzpicture}
    \node[draw=secondary,fill=lightbg,rounded corners=4pt,
          inner sep=18pt,text width=8.5cm,align=center] {
        {\large\bfseries\color{primary} 崔敬然}\\[4pt]
        {\color{secondary} 学号：23336055}\\[2pt]
        {\color{secondary} 班级：1班}\\[6pt]
        {\color{accent}\today}
    };
\end{tikzpicture}
\end{center}
...
\vspace*{4cm}
\begin{center}
{\color{primary}\LARGE\bfseries 摘\quad 要}\\[14pt]
\color{secondary}\rule{0.35\textwidth}{0.6pt}
\end{center}
\vspace{1.2cm}
...
\vspace*{4cm}
\begin{center}
{\color{primary}\LARGE\bfseries Abstract}\\[14pt]
\color{secondary}\rule{0.35\textwidth}{0.6pt}
\end{center}
\vspace{1.2cm}
\begin{minipage}{0.88\textwidth}
...
\vspace*{2cm}
\begin{center}
{\color{primary}\LARGE\bfseries 目\quad 录}\\[14pt]
\color{secondary}\rule{0.35\textwidth}{0.6pt}
\end{center}
\vspace{1.2cm}
...
\begin{tikzpicture}[
    ...
    input/.style={rectangle, draw=secondary!60, fill=lightbg, rounded corners=3pt, minimum width=28mm, minimum height=11mm, align=center, font=\footnotesize},
    proc/.style={rectangle, draw=secondary!50, fill=secondary!5, rounded corners=3pt, minimum width=28mm, minimum height=10mm, align=center, font=\footnotesize},
    core/.style={rectangle, draw=primary!80, fill=primary!8, rounded corners=3pt, minimum width=105mm, minimum height=12mm, align=center, font=\small\bfseries\color{primary}},
    outstyle/.style={rectangle, draw=accent!60, fill=accent!8, rounded corners=3pt, minimum width=105mm, minimum height=11mm, align=center, font=\small\bfseries\color{greencol}},
    ui/.style={rectangle, draw=warncol!60, fill=warncol!5, rounded corners=3pt, minimum width=30mm, minimum height=16mm, align=center, font=\footnotesize},
...
\caption{核心模型选型与技术参数}
\label{tab:models}
\rowcolors{2}{tabrowb}{tabrowa}
\begin{tabular}{@{}lll@{}}
\toprule
\rowcolor{tabhead}
\color{white}\textbf{系统组件} & \color{white}\textbf{模型/方案} & \color{white}\textbf{关键参数} \\
\midrule
...
\begin{keybox}[三模态协同控制的核心设计原则]
ControlNet作用于\textbf{像素/空间层面}（构图引导），IP-Adapter作用于\textbf{语义/风格层面}（外观调制），文本提示词提供\textbf{语义/内容层面}（主题约束）。三者控制维度天然正交，在SDXL UNet内部实现互不冲突的潜在空间融合。
\end{keybox}
...
\begin{tipbox}
\textbf{工程实践建议：} CPU卸载（\texttt{enable\_model\_cpu\_offload}）是本方案中最关键的单项优化，单独可将静态显存从\textgreater20\,GB降至\textasciitilde10\,GB。VAE切片/瓦片优化对峰值显存的削减效果显著（约60\%），但对推理速度有轻微影响（增加1--2秒）。建议默认开启全部优化选项以保证稳定性。
\end{tipbox}
...
\toprule
\rowcolor{tabhead}
\color{white}\textbf{主题} & \color{white}\textbf{提示词核心要素} & \color{white}\textbf{风格参考图特征} \\
\midrule
...
\toprule
\rowcolor{tabhead}
\color{white}\textbf{实验组} & \color{white}\textbf{ControlNet} & \color{white}\textbf{IP-Adapter} & \color{white}\textbf{CN权重} & \color{white}\textbf{IP权重} \\
\midrule
...
\toprule
\rowcolor{tabhead}
\color{white}\textbf{主题} & \color{white}\textbf{G1 纯文本} & \color{white}\textbf{G2 +CN} & \color{white}\textbf{G3 +IP} & \color{white}\textbf{G4 完整} \\
\midrule
...
\toprule
\rowcolor{tabhead}
\color{white}\textbf{指标} & \color{white}\textbf{G1 基线} & \color{white}\textbf{G2 +CN} & \color{white}\textbf{G3 +IP} & \color{white}\textbf{G4 完整} \\
\midrule
...
\begin{keybox}[FID指标的解读要点]
FID衡量的是\textbf{分布差异}而非\textbf{绝对质量}。完整多模态系统G4的FID最高，恰恰说明ControlNet+IP-Adapter联合作用对生成分布的重塑最为显著——这种重塑是\textbf{正向的}（G4在CLIP Score和美学得分上均为最优）。不应将高FID误解为质量下降。
\end{keybox}
...
\toprule
\rowcolor{tabhead}
\color{white}\textbf{实验组} & \color{white}\textbf{美观度} & \color{white}\textbf{布局合理度} & \color{white}\textbf{风格表现力} \\
\midrule
...
\vspace*{2cm}
\begin{center}
{\color{primary}\LARGE\bfseries 参考文献}\\[14pt]
\color{secondary}\rule{0.35\textwidth}{0.6pt}
\end{center}
\vspace{1.2cm}
...
\begin{lstlisting}[style=pycode, caption={PosterEngine核心生成引擎（engine.py，关键代码节选）}]
class PosterEngine:
...
\begin{lstlisting}[style=pycode, caption={风格参考图程序化生成（evaluation.py节选）}]
...
\begin{lstlisting}[style=pycode, caption={CLIP Score计算（evaluation.py节选）}]
class CLIPScorer:
    def __init__(self):
...
\toprule
\rowcolor{tabhead}
\color{white}\textbf{文件} & \color{white}\textbf{说明} \\
\midrule
